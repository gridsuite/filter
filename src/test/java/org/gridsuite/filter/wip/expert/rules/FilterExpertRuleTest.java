...
class FilterExpertRuleTest {
    ...
    private static Stream<Arguments> provideInvalidParameters() {
        return Stream.of(
                ...
                Arguments.of(OperatorType.EQUALS, null),
                Arguments.of(OperatorType.EQUALS, Collections.emptySet()),
                Arguments.of(OperatorType.EQUALS, Set.of(mock(AbstractFilter.class)))
        );
    }
    ...
    void testInvalidParameters(OperatorType operatorType, Set<Filter> referenceFilters) {
        assertThatThrownBy(() -> FilterExpertRule.of(operatorType, referenceFilters)).isInstanceOf(PowsyblException.class);
    }

    @Test
    void testValidParameters() {
        assertThatNoException().isThrownBy(() -> FilterExpertRule.of(OperatorType.IS_PART_OF, Collections.singleton(mock(AbstractFilter.class))));
    }

    @Test
    void testGetDataTypeReturnsFilter() {
        FilterExpertRule rule = FilterExpertRule.of(OperatorType.IS_PART_OF, Collections.singleton(mock(AbstractFilter.class)));
    ...
    void testMockRuleEvaluationReturnsExpected(OperatorType operatorType, Set<Filter> referenceFilters, String equipmentId, boolean expectedResult) {
        ExpertRule rule = FilterExpertRule.of(operatorType, referenceFilters);
